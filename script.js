...
const translations = {
  es: {
    ...
    "projects.ultraBingoTitle": "Ultra Bingo — Juego Multijugador Web3 en Tiempo Real",
    "projects.ultraBingoDesc": "Desarrollé una plataforma de bingo multijugador con sincronización en tiempo real mediante Socket.io, permitiendo partidas simultáneas con validación instantánea de ganadores. Implementé micropagos por tarjeta usando el protocolo x402 (HTTP 402 Payment Required) con integración de wallets Web3. El backend incluye autenticación JWT, rate limiting, audit logging y verificación de integridad de tarjetas mediante hashing. Stack completo: React 19, Node.js, MongoDB, con arquitectura event-driven para escalabilidad.",
    "projects.ultraPayTitle": "UltraPay x402 — Plataforma de Generación AI con Micropagos",
    "projects.ultraPayDesc": "Arquitecté una plataforma serverless de generación de imágenes y videos con IA, monetizada mediante el estándar HTTP 402 Payment Required. Los usuarios pagan por prompt ($0.10 imágenes, $0.25 videos) en USDC sin suscripciones. Implementé routing multi-modelo (FLUX.1, preparado para Veo 3, Runway, Midjourney), almacenamiento en S3 con URLs pre-firmadas, y toda la infraestructura desplegada con Terraform. El sistema procesa pagos on-chain verificados por el facilitador x402 de UltravioletaDAO."
  },
  en: {
    // Navigation
    "nav.resume": "Resume",
    ...
    "projects.ultraBingoTitle": "Ultra Bingo — Real-Time Web3 Multiplayer Game",
    "projects.ultraBingoDesc": "Developed a multiplayer bingo platform with real-time synchronization via Socket.io, enabling simultaneous games with instant winner validation. Implemented per-card micropayments using the x402 protocol (HTTP 402 Payment Required) with Web3 wallet integration. Backend includes JWT authentication, rate limiting, audit logging, and card integrity verification through hashing. Full stack: React 19, Node.js, MongoDB, with event-driven architecture for scalability.",
    "projects.ultraPayTitle": "UltraPay x402 — AI Generation Platform with Micropayments",
    "projects.ultraPayDesc": "Architected a serverless AI image and video generation platform, monetized via the HTTP 402 Payment Required standard. Users pay per prompt ($0.10 images, $0.25 videos) in USDC with no subscriptions. Implemented multi-model routing (FLUX.1, ready for Veo 3, Runway, Midjourney), S3 storage with pre-signed URLs, and all infrastructure deployed with Terraform. The system processes on-chain payments verified by UltravioletaDAO's x402 facilitator."
  }
};
